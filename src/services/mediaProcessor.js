/**
 * MediaProcessor - Versione ottimizzata per l'estrazione articolo/prezzo
 */
...
class MediaProcessor {
  constructor(telegramService) {
    ...
    this.markupPercentage = 25; // Markup fisso al 25%
  ...
  async processMessage(message, channelConfig) {
    try {
      ...
      if (message.message && channelConfig.includeText) {
        // Estrai solo l'articolo dal testo
        ...
        if (channelConfig.includePrice) {
          const extractedPrice = contentCleaner.extractPrice(
            message.message,
            channelConfig.priceRegex
          );
          if (extractedPrice) {
            processedContent.price = contentCleaner.applyMarkup(
              extractedPrice,
              this.markupPercentage
            );
            logger.debug(
              `Prezzo estratto: ${JSON.stringify(processedContent.price)}`
            );
          }
        }
      }
    ...
    for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
      try {
        ...
        if (!originalMessage.media) {
          ...
          logger.info(
            `Messaggio di solo testo inviato nel topic ${channelConfig.destinationTopic}`
          );
          return true;
        }

        // Altrimenti, gestisci il media
        const mediaBuffer = await userClient.downloadMedia(
        ...
        const filePath = await this.saveTempMediaFile(
          mediaBuffer,
          originalMessage.media.className
        );
        ...
        logger.info(
          `Media inviato con successo nel topic ${channelConfig.destinationTopic}`
        );
        return true;
      } catch (error) {
        logger.warn(
          `Tentativo ${attempt}/${this.retryAttempts} fallito: ${error.message}`
        );
   ...
   * Solo articolo e prezzo con markup, senza indicazione di markup
   */
  ...
  async publishMediaGroup(groupData, channelConfig) {
    try {
      ...
      if (
        !groupData.originalMessages ||
        groupData.originalMessages.length === 0
      ) {
      ...
      if (firstMsg && firstMsg.media) {
        ...
        const filePath = await this.saveTempMediaFile(
          mediaBuffer,
          firstMsg.media.className
        );

        // Invia il primo messaggio con tutto il testo
        const sentMessage = await this.sendPhoto(
          filePath,
          caption,
          channelConfig.destinationTopic
        );
          ...
          for (let i = 1; i < groupData.originalMessages.length; i++) {
            ...
            if (msg && msg.media) {
              try {
                ...
                await this.cleanupTempFile(filePath);
                logger.info(
                  `Immagine ${i + 1} del gruppo inviata come risposta`
                );
              } catch (err) {
      ...
      logger.error(
        `Errore nella pubblicazione del gruppo di media: ${error.message}`
      );
      return false;
    }
  }
